...
@Component
public class CsvToSqlServerWPulseInvitationsExport {
    ...
    public void convertCsvToSqlServer() {
        try {
            ...
            try {
                CSVReader csvReader = new CSVReader(new FileReader(this.inputFilePathwm_wm_invitations));
                try {
    ...
    private String buildInsertionSql(String[] headers) {
        String sql = "INSERT INTO " + this.tableNamewPulse_Invitation_Export + " VALUES (";
    ...
    public List<ConsultaResultado> realizarConsultas(LocalDate fechaConsulta) {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcUrl)) {
            resultados.add(consultarTabla(connection, tableNamewPulse_Invitation_Export, fechaConsulta));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return resultados;
